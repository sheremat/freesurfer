function [vol, M0_vox2ras, M0_vox2tkr] = mris_read_mgh(filename)
...
% $Id: mris_read_mgh.m,v 1.4 2012/12/22 00:20:41 jonp Exp $
%**************************************************************************%
  ...
  [vol, M0_vox2ras, M0_vox2tkr] = load_mgh_local(filename);
...
%**************************************************************************%
function [vol, M, M0_vox2ras_tkr] = load_mgh_local(fname,slices,frames,headeronly)
...
if (ras_good_flag)
  ...
  % LOCAL VERSION:
  M0_vox2ras_tkr = [[-delta(1), 0, 0; 0, 0, +delta(3); 0 -delta(2) 0], [+Pcrs_c(1);-Pcrs_c(2);+Pcrs_c(3)]; [0 0 0 1]];
...
return;
  %%% $Source: /usr/fscvsroot/dev/gradient_nonlin_unwarp/mris_read_mgh.m,v $
  %%% Local Variables:
  %%% mode: Matlab
  %%% fill-column: 76
  %%% comment-column: 0
